...
export default async function ContractsPage() {
  ...
  const result = await getOrgContracts(user.id, activeOrgId);

  return (
    <div className="px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
      <PageHeader
        title="Contracts"
        description="View and manage all client contracts across your organization."
      />
      ...
      {result.error || !result.data ? (
        ...
        <ContractTable contracts={result.data.contracts} />
      )}
    </div>
  );
}
