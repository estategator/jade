...
export type ProfileRole = 'user' | 'admin' | 'developer';
...
export async function getAllTickets(
  ...
  if (!dev) return { error: 'Access denied.' };
...
export async function getTicketStats(userId: string): Promise<{
  ...
  const dev = await isDeveloper(userId);
  if (!dev) return { total: 0, open: 0, in_progress: 0, resolved: 0, closed: 0, error: 'Access denied.' };
